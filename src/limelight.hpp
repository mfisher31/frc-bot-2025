...
inline frc::Pose3d toPose3D (const std::vector<double>& inData) {
    ...
    return frc::Pose3d (
        ...
        frc::Rotation3d (units::angle::radian_t (inData[3] * (M_PI / 180.0)), units::angle::radian_t (inData[4] * (M_PI / 180.0)), units::angle::radian_t (inData[5] * (M_PI / 180.0))));
}
...
public:
    ...
    PoseEstimate (const frc::Pose2d& pose, units::time::second_t timestampSeconds,
        ...
        : pose (pose), timestampSeconds (timestampSeconds), latency (latency), tagCount (tagCount), tagSpan (tagSpan), avgTagDist (avgTagDist), avgTagArea (avgTagArea), rawFiducials (rawFiducials) {
    }
};
